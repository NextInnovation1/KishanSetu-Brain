...
Everything is served by one backend: Node.js 20 + Express, PostgreSQL 16, REST JSON snake_case, JWT + phone-OTP auth ([06-PRD-BACKEND.md](06-PRD-BACKEND.md), [11-ARCHITECTURE.md](11-ARCHITECTURE.md)).
...
| Success looks like | A payout notification with a UPR/UTR number the same morning he delivered, for visibly more money than the mandi rate. |

Product consequences: Hindi-first UI with English subtitles (Gujarati in v1.1), max 2 actions per screen, ≥48dp touch targets, picture grids instead of dropdowns, every money figure shown as *reference market price struck through vs KisanSetu price in green* — the trust arithmetic is the interface.

### 3.2 Kiranben — FPO hub operator (aggregation)

| | |
|---|---|
| Profile | 35, FPO field staff running the collection hub near Olpad; manages intake mornings 6–10 AM. |
...
| Jobs-to-be-done | (1) "Process 40 farmers' intake in 4 hours without a queue riot." (2) "Grade consistently so neither farmer nor buyer disputes it." (3) "Know exactly which crates go on which truck." |
...
| Success looks like | Grade A tomatoes at a stable published price, delivered before 11 AM, with a trace-line showing the farm, village and harvest time. |
...
| Jobs-to-be-done | (1) "Set tomorrow's A/B prices from the reference market price by 6 PM." (2) "Match graded supply to placed orders with zero unfulfilled surprises." (3) "Fire farmer payouts on pickup and keep the SLA clock honest." |
...
4. **Mid-morning (D0):** 3PL truck picks up. Allocation to buyer orders happened overnight/against the morning intake. Payout fires on pickup: UPI credit + notification **पैसा आ गया ✓** with amount and UTR. Printed payout slip shows mandi rate vs what he got — the slip is the advertisement.
...
3. **Night (D-1) → morning (D0):** Order status timeline moves: **placed → confirmed** (ops matched supply) → **out_for_delivery** (truck dispatched) → **delivered** before 11 AM.
4. **On delivery:** One invoice (number on the order detail). He opens **Order Detail → trace line**: Farm (Ramesh, Olpad village) → harvested 6:10 AM → hub in 8:05 AM → dispatched 9:20 AM → delivered 10:48 AM. He shows a customer where lunch came from.
...
1. **6:00 AM:** Opens the grading queue on the ops dashboard — sees all listings posted for today, sorted by farmer arrival.
...
3. **9:30 AM:** Allocation board matches graded listings to order items (FIFO suggestion, ops confirms). Crates staged per buyer route.
4. **10:00 AM:** 3PL truck arrives; she marks dispatch (sets `dispatch_ts`); farmer payouts fire on pickup.
...
4. **Midday:** Confirms deliveries; triggers payouts; chases any stuck `out_for_delivery`.
...
| **2. Global-first, Surat-first** | i18n string tables from day 1 (EN/HI at launch, GU v1.1, pluggable); `currency` on every money field; E.164 phones; TZ-aware timestamps; `region → hub → farm` hierarchy; `reference_market_price` in the schema ("mandi" only in Indian UI copy); per-region tax abstraction (GST is an instance). |
...
- **No in-app payments at MVP** — farmer payouts are ops-triggered server-side (UPI via abstracted provider; Razorpay is the India instance); buyers pay on invoice. In-app collection adds PSP + app-store complexity before we've validated the trade itself.
...
|---|---|---|
| **Phase 0 — validation** | None (docs + sales deck) | ≥3/10 buyers willing to switch, 1 FPO MoU, crops frozen — else pivot to FPO-SaaS-first ([13-LAUNCH-PLAN.md](13-LAUNCH-PLAN.md)) |
| **MVP (pilot)** | Ops dashboard → mobile apps (both platforms) → website | 1 hub, 2 crops, 25–50 farmers, 15–25 buyers |
| **v1.1** | + Gujarati, offline queue, push (apps); SLA counter (website) | Pilot go/no-go: ≥70% buyer weekly repeat, ≥95% fulfilment, farmer share ≥60% documented |
