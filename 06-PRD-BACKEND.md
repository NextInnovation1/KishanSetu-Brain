...
The tables below are the **authoritative logical schema**. They are implemented as **Drizzle table definitions** in `src/db/schema/*` (TypeScript), and **`drizzle-kit generate`** emits the versioned SQL migrations in `src/db/migrations/` (applied with `drizzle-kit migrate`) — this replaces the old hand-written numbered `.sql` + `schema_migrations` approach. `pgcrypto` for `gen_random_uuid()`. All PKs `uuid` (supersedes the scaffold's `serial` produce id). The DDL here is what the Drizzle schema must produce; where the SQL and a Drizzle column differ, this DDL wins and the schema is corrected. See [20-CODE-ARCHITECTURE.md](20-CODE-ARCHITECTURE.md) §1.2.
...
**Security**: TLS terminated at reverse proxy (Caddy) — API never listens publicly on plain HTTP in prod. `helmet` headers; CORS allowlist = website + ops dashboard origins only (mobile apps don't need CORS). Parameterized queries exclusively — **Drizzle** binds all parameters; any hard analytics SQL uses Drizzle's `sql`-tagged template (never string concatenation; repo lint greps for raw interpolation). JWT HS256 with ≥32-byte secret; secrets only via env (never in repo); `.env.example` documents every var. OTP codes hashed (sha256 + `OTP_PEPPER`). PII minimization: trace endpoints expose farmer first name + village only; logs redact `phone`, `upi_vpa`, `tax_id`, `utr`. All ops mutations write `audit_events`. Payout mutation endpoints (M3–M5) additionally log full request bodies to audit. Dependency policy: `npm audit` in CI, Express + `drizzle-orm`/`drizzle-kit` + `pg` + `zod` + `jsonwebtoken` + `helmet` + `pino` and little else. TypeScript strict mode on. Backups encrypted at rest.
