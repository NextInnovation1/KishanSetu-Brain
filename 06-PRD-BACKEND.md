...
| **Heavy ORM (Prisma/Sequelize/TypeORM)** | **Data layer = Knex + Objection.js** (the Fancall canon — [20-CODE-ARCHITECTURE.md](20-CODE-ARCHITECTURE.md) §1). Knex owns migrations + the hard queries (allocation matching, SLA medians, farmer share — write these as Knex query-builder / raw where clearer); Objection models add soft-delete + timestamp hooks via an `AppModel` base. This supersedes the earlier "raw `pg`, numbered `.sql`" plan — migrations are now Knex files. Prisma/Sequelize/TypeORM specifically are still out (Objection is lighter and matches the reference codebase). |
