...
**Mitigation:** per-trip contracts with rate cards, no retainers ([14-OPS-PLAYBOOK.md](14-OPS-PLAYBOOK.md) §6); two trial routes before signing; vendor #2 kept warm for price tension; delivery-window batching (05:30–09:30) to maximize drops per trip; insulated-box standard instead of premature reefer; logistics fee passed to buyer as an explicit line item so cost inflation is visible, not silently margin-eating.
...
**Contingency:** bridge via Gujarat angel/family-office networks (smaller cheques, faster); slow expansion, not the core (Surat alone can be profitable at ~50–75 buyers per [03-BUSINESS-MODEL.md](03-BUSINESS-MODEL.md)); venture debt/revenue-based financing against recurring buyer revenue if metrics support it.
...
**Mitigation:** ₹1 test payout verifies every farmer VPA at onboarding (T6.4); payouts only against graded listings (no free-form transfers); ops device with screen lock + separate Razorpay role credentials; buyer credit capped at net-7 and only after 2 weeks of prepaid history; weekly reconciliation of payouts vs slips vs bank statement.
**Contingency:** single-incident cap ~₹10k at pilot AOVs; Razorpay dispute process; buyer defaults handled by supply suspension (the strongest B2B collections tool is tomorrow's delivery). Not a kill-level risk at pilot scale; re-scored before scale-up.
...
**Mitigation:** 2 crops chosen partly for complementary seasonality (T0.6 criterion); FPO field signal gives 48h warning (§5 reference data #3); the movement damper + honest partial-fill SOP handles day-scale shocks; glut days are farmer-acquisition days (we absorb volume at market-tracking prices when the mandi punishes them).
